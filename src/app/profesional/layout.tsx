...
import { useRequireProfessional } from "@/hooks/useRequireProfessional";
...
export default function ProfesionalLayout({ children }: { children: React.ReactNode }) {
  const { loading, isProfessional } = useRequireProfessional();
  const [sidebarOpen, setSidebarOpen] = useState(false);

  if (loading || !isProfessional) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-lavender-light">
        <div className="h-8 w-8 rounded-full border-4 border-green border-t-transparent animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-lavender-light">
      ...
      <ProfesionalSidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} />
